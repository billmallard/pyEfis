...
class Screen(QWidget):
    def __init__(self, parent=None, config=None):
        ...
        self.display_state_controller = screenbuilder_display.DisplayStateController(
            self
        )
        ...
        for inst, this_replacements, state in expanded_instruments:
            if "ganged" in inst["type"]:
                ...
                if "gang_type" not in inst:
                    raise Exception(
                        f"Instrument {inst['type']} must also have 'gang_type:' horizontal|vertical specified"
                    )
                self.instrument_config[count] = inst
                for g in inst["groups"]:
                    for gi in g["instruments"]:
                        gi["type"] = inst["type"].replace("ganged_", "")
                        gi["options"] = g.get("common_options", dict()) | gi.get(
                            "options", dict()
                        )  # Merge with common_options losing the the instrument
                        self.setup_instruments(
                            count,
                            gi,
                            ganged=True,
                            replace=this_replacements,
                            state=state,
                        )
                ...
                self.setup_instruments(
                    count, inst, replace=this_replacements, state=state
                )
                ...
                if isinstance(inst_disabled, bool) and inst_disabled == True:
                    ...
                    isinstance(inst_disabled, str)
                    and not self.parent.preferences["enabled"][inst_disabled]
                ):
                    self.instruments[count].setVisible(False)
                else:
                    if state:
    ...
    def get_grid_margins(self):
        return screenbuilder_layout.get_grid_margins(
        ...
        for i, c in self.instrument_config.items():
            ...
            if "ganged" in c["type"]:
                ...
                if hasattr(self.instruments[i], "getRatio"):
                    ...
                    logger.debug(
                        f"Ganged Instrument {c['type']} has ratio 1:{ganged_ratio}"
                    )

                for gang_count, ganged_geometry in enumerate(
                    screenbuilder_layout.get_ganged_geometries(
                        c, x, y, width, height, ganged_ratio
                    )
                ):
    ...
    def get_bounding_box(self, width, height, x, y, ratio):
        r_width, r_height, r_x, r_y = screenbuilder_layout.get_bounding_box(
            width, height, x, y, ratio
        )
        logger.debug(
            f"x:{x}, r_x:{r_x} y:{y} r_y:{r_y} width:{width} r_width:{r_width} height:{height} r_height:{r_height}"
        )
