...
- **Log a manual test case for EVERY visual edge case** (label `test-case`; "Visual / SVS Test Case" template lives on branch `test-case-templates`, needs merging to master to activate). #41/#43 are the first two. Always record heading + whether the feature is inside the forward fan for the viewport aspect under test (see the HFOV relation below — there is no fixed fan width).
- **SVS rendering facts that masquerade as bugs:** terrain is drawn in a **forward fan set by the AI viewport's aspect ratio, not a fixed angle** — `HFOV_deg = viewport_width * _TERRAIN_HFOV_PITCH_DEG / viewport_height` (`_TERRAIN_HFOV_PITCH_DEG = 30`, `svs_gl.py`, applied to x in `camera.py:85-86` via `pixels_per_deg_h`). **AER-1973 (2026-09-23) decoupled this from `pitchDegreesShown`** (now 50, was 30, `ai_widget.py`) — `pitchDegreesShown` sets only the pitch ladder's vertical gain today; the terrain HFOV is pinned separately and does NOT change when `pitchDegreesShown` changes. Features behind/beside you aren't drawn (check heading vs bearing-to-feature against that widget's actual HFOV FIRST, not a remembered constant — e.g. ~50° on an 800x480 panel, 51.2° on 1024x600, 40° for `tools/svs_capture.py`'s 800x600 default — these numbers are unchanged by AER-1973 since the pin is still 30). `fov_deg` in the old `POLAR_DEFAULTS` was dead config for the legacy polar/CPU tiers and has been deleted (AER-1478) — the GL path never had an azimuth cull of its own. **No depth test** — overlays paint on top of terrain in painter's order (no z-fighting). Inland water with `elev_ft=NULL` is drawn at the terrain elevation sampled at **vertex 0 only** (`_collect_water_sync`). GLO-30 flattens lakes to their surface (valid, not void).
- **SIGUSR1 screenshot behavior differs by platform (AER-2002, fixed):** `kill -USR1 <pid>` → `/tmp/pyefis_screenshot.png` grabs `gui.mainWindow`. On the **Beelink (X11)** the live display is unaffected; only the captured image shows a blank SVS/AI panel (a `QOpenGLWidget`, GL doesn't render into an offscreen grab — expected, not a fault). On the **Pi 5 (eglfs)** the same grab used to black out the *live* SVS until restart — root-caused to the Pi's CMA pool sitting at ~0 free in steady state (`CmaTotal 65536 kB`, `CmaFree 16 kB`; corroborated by pre-existing `/tmp/svs_gl_failure.log GL_OUT_OF_MEMORY` entries unrelated to any screenshot), so Qt's internal GL-widget grab path — an FBO operation pyEfis code has no hook into — had no memory headroom and no failure recovery. Fixed by skipping the grab entirely under eglfs (`main.py`, `app.platformName() == "eglfs"`): a SIGUSR1 there now logs and no-ops instead of writing a file. Don't re-enable a Pi screenshot without solving the CMA headroom first.
