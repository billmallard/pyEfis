...
- **SVS rendering facts that masquerade as bugs:** terrain is drawn in a **forward fan set by the AI viewport's aspect ratio, not a fixed angle** — `HFOV_deg = viewport_width * pitchDegreesShown / viewport_height` (`pitchDegreesShown = 30`, `ai_widget.py:488`, applied to x in `camera.py:85-86`); features behind/beside you aren't drawn (check heading vs bearing-to-feature against that widget's actual HFOV FIRST, not a remembered constant — e.g. ~50° on an 800x480 panel, 51.2° on 1024x600, 40° for `tools/svs_capture.py`'s 800x600 default). `fov_deg` in the old `POLAR_DEFAULTS` was dead config for the legacy polar/CPU tiers and has been deleted (AER-1478) — the GL path never had an azimuth cull of its own. **No depth test** — overlays paint on top of terrain in painter's order (no z-fighting). Inland water with `elev_ft=NULL` is drawn at the terrain elevation sampled at **vertex 0 only** (`_collect_water_sync`). GLO-30 flattens lakes to their surface (valid, not void).
