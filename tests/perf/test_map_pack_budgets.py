...
def raleigh_volume_landscape(bench, qapp):
    ...
    _root, tiles, water, _hw = _scene_pack("raleigh")
    _require_footprint("raleigh", water, 160.0, w=800, h=480)
    out = M.measure_water_volume(bench, qapp, "raleigh", tiles, water,
                                 w=800, h=480)
    if out is None:
...
def _require_footprint(scene, water_db, range_nm, w=M.SCENE_W, h=M.SCENE_H):
    ...
    cov = M.footprint_covers_window(
        M.water_coverage_bbox(water_db), s["lat"], s["lon"], range_nm, w, h)
    if not cov["covers"]:
        pytest.skip(
