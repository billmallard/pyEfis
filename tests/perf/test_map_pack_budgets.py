...
def _require_footprint(scene, water_db, range_nm, pack_root, w=M.SCENE_W,
                       h=M.SCENE_H):
    ...
    AER-1149: *w* and *h* default to the suite's own 650x1040 (section
    5's geometry), but thread through to ``pack_coverage`` so a caller
    at another aspect -- the landscape fixture above, 800x480 -- asks
    the coverage question at the geometry it actually renders at rather
    than the portrait one. This was the "known residual" this docstring
    used to flag: a pack cut exactly to the 160 NM portrait window
    (7.85 x 9.68 deg) reads ``covers=True`` at 650x1040 and
    ``covers=False`` at 800x480 (12.95 x 15.96 deg) -- the same false
    pass in a different variable, which is why the default alone was
    never going to be enough once a non-portrait row existed."""
    s = M.SCENES[scene]
